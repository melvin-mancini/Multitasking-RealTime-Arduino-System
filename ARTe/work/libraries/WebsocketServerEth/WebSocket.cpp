...
void WebSocketServer::listen() {
    ...
    for( byte x=0; x < m_maxConnections; x++ )
    {
        ...
        {
#ifdef DEBUG
			Serial.println("Socket DELETED");	//ARTE
#endif
			m_connectionCount--;
            delete s;
            m_connections[x] = NULL;
...
#ifdef DEBUG
			Serial.print("WSS listen: socket ");
			Serial.print(x);
    ...
    if( !cli )
		...
		for ( byte x=0; x < m_maxConnections; x++ )
			if (m_connections[x] && m_connections[x]->client == cli)
				return;
	}

#ifdef DEBUG
	Serial.println(F("WSS listen: Client Avaiable.")); //ARTE
#endif
    ...
    client(cli)
{
    if( doHandshake() )
    {
        state = CONNECTED;
#ifdef DEBUG
		Serial.println("Do handshake TRUE, state CONNECTED"); //ARTE
#endif
...
void WebSocket::listen()
{
    if( !client.available() ) {
#ifdef DEBUG
		Serial.println("listen: client: no data available"); //ARTE
#endif
        return;
	}
...
#ifdef DEBUG
		Serial.println("listen: getFrame 0"); //ARTE
#endif
        disconnectStream();
	}
}
...
bool WebSocket::doHandshake() {
    ...
    if (hasUpgrade && hasConnection && isSupportedVersion && hasHost && hasKey) {
        strcat(key, PSTR("258EAFA5-E914-47DA-95CA-C5AB0DC85B11")); // Add the omni-valid GUID
        Sha1.init();
        Sha1.print(key);
