 ...
 * CVS: $Id: ee_queue_DM.c,v 1.2 2005/07/17 13:58:36 pj Exp $
 */

#include "com/com/inc/ee_comprv.h"

/* This function insert a new monitored ipdu in deadline montor queue. 
...
void EE_com_insert_DM (MessageIdentifier ipdu_num, EE_UINT8 reload)
{
  ...
  cont = reload;
  ipdu_curr = EE_com_sys.first_DM;
      
  if (ipdu_curr == EE_COM_NULL) 
  { /* The ipdu become the first into the queue */
    EE_com_sys.first_DM = ipdu_num;
    EE_com_ipdu_RAM[ipdu_num]->cont_DM = cont;
  }
  else 
    if (cont < EE_com_ipdu_RAM[ipdu_curr]->cont_DM)
    { /* The ipdu however become the first into the queue */
      EE_com_ipdu_RAM[ipdu_curr]->cont_DM -= cont;
      EE_com_ipdu_RAM[ipdu_num]->cont_DM = cont;
      EE_com_sys.first_DM = ipdu_num; 
    }
    else
    { /* I have to search the right place */
      do { 
        cont -= EE_com_ipdu_RAM[ipdu_curr]->cont_DM;
        ipdu_prev = ipdu_curr;       
        ipdu_curr = EE_com_ipdu_RAM[ipdu_curr]->next_DM;
      ...
      EE_com_ipdu_RAM[ipdu_prev]->next_DM = ipdu_num;
    }
    
  EE_com_ipdu_RAM[ipdu_num]->next_DM = ipdu_curr;
...
void EE_com_remove_DM (MessageIdentifier ipdu_num)
{
  MessageIdentifier ipdu_prev, ipdu_curr;
  ...
  { /* The element that I have to be extract is in the first position */
    EE_com_sys.first_DM = EE_com_ipdu_RAM[ipdu_curr]->next_DM;
  }
  else
  {
    do {
    ...
    EE_com_ipdu_RAM[ipdu_prev]->next_DM = 
          EE_com_ipdu_RAM[ipdu_curr]->next_DM;
  }
    
  EE_com_ipdu_RAM[EE_com_ipdu_RAM[ipdu_curr]->next_DM]->cont_DM += 
        EE_com_ipdu_RAM[ipdu_curr]->cont_DM;
}
#endif
