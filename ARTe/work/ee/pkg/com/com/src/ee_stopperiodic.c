 ...
 * CVS: $Id: ee_stopperiodic.c,v 1.2 2005/07/17 13:58:36 pj Exp $
 */
...
/* The idea is to stop any further execution of pmonitor function,
 * setting to NULL the head of periodic queue
 */
  ...
  for (i=0; i<EE_COM_N_IPDU; i++) 
  {
    ...
    if ((IPDU_RAM->property & (EE_MASK_IPDU_TXRX | EE_MASK_IPDU_DPM)) 
      ...
      if ((IPDU_RAM->property & EE_MASK_IPDU_DMTM) == 
            EE_COM_IPDU_DM_SET)
        EE_com_remove_DM (EE_com_ipdu_ROM[i]->name);
      else if ((IPDU_RAM->property & EE_MASK_IPDU_DMTM) == 
            EE_COM_IPDU_TM_SET)
